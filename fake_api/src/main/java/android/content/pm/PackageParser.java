...
public class PackageParser {
  ...
  public static class PackageLite {
    ...
    /**
     * Path where this package was found on disk. For monolithic packages this is path to single
     * base APK file; for cluster packages this is path to the cluster directory.
     */
  ...
  public static class ApkLite {
    ...
    public ApkLite(
        ...
        final List<VerifierInfo> verifiers,
  ...
  /**
   * Parse only lightweight details about the package at the given location. Automatically detects
   * if the package is a monolithic style (single APK file) or cluster style (directory of APKs).
   *
   * <p>This performs sanity checking on cluster style packages, such as requiring identical package
   * name and version codes, a single base APK, and unique split names.
   *
   * @see PackageParser#parsePackage(File, int)
   */
  public static PackageLite parsePackageLite(final File packageFile, final int flags)
   ...
   * name and version codes, a single base APK, and unique split names.
   *
   * <p>Note that this <em>does not</em> perform signature verification; that must be done
   * separately in {@link #collectCertificates(Package, int)}.
   *
   * @see #parsePackageLite(File, int)
   * @since Android 5.0+
   */
  public Package parsePackage(final File packageFile, final int flags)
      throws PackageParserException {
    throw new RuntimeException("Stub!");
  }
  ...
  public void collectCertificates(final Package pkg, final int flags)
      throws PackageParserException {
    throw new RuntimeException("Stub!");
  }

  /**
   * Utility method that retrieves lightweight details about a single APK file, including package
   * name, split name, and install location.
   *
   * @param apkFile path to a single APK
   * @param flags optional parse flags, such as {@link #PARSE_COLLECT_CERTIFICATES}
   */
  public static ApkLite parseApkLite(final File apkFile, final int flags)
      throws PackageParserException {
    throw new RuntimeException("Stub!");
  }

  /**
   * Representation of a full package parsed from APK files on disk. A package consists of a single
   * base APK, and zero or more split APKs.
   */
  public static final class Package implements Parcelable {
     ...
     *
     * <p>TODO: This seems like a horrible place to put the abiOverride because this isn't something
     * the packageParser parsers. However, this fits in with the rest of the PackageManager where
     * package scanning randomly pushes and prods fields out of {@code this.applicationInfo}.
     */
    ...
    public static final Creator<Package> CREATOR =
        new Creator<Package>() {
          @Override
    ...
    /** Filtered set of {@link #getAllCodePaths()} that excludes resource-only APKs. */
    ...
    public void writeToParcel(Parcel dest, int flags) {
      ...
      dest.writeByte((byte) (baseHardwareAccelerated ? 1 : 0));
      ...
      dest.writeTypedList(preferredActivityFilters);
      ...
      dest.writeLong(mLastPackageUsageTimeInMills);
      dest.writeTypedList(configPreferences);
      dest.writeTypedList(reqFeatures);
      dest.writeTypedList(featureGroups);
      dest.writeInt(installLocation);
      dest.writeByte((byte) (coreApp ? 1 : 0));
      dest.writeByte((byte) (mRequiredForAllUsers ? 1 : 0));
      dest.writeString(mRestrictedAccountType);
      dest.writeString(mRequiredAccountType);
      dest.writeParcelable(manifestDigest, flags);
      dest.writeString(mOverlayTarget);
      dest.writeInt(mOverlayPriority);
      dest.writeByte((byte) (mTrustedOverlay ? 1 : 0));
