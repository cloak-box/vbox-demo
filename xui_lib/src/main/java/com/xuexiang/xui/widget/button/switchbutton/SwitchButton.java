...
public class SwitchButton extends CompoundButton {
  ...
  private static int[] CHECKED_PRESSED_STATE =
      new int[] {
        android.R.attr.state_checked, android.R.attr.state_enabled, android.R.attr.state_pressed
      };
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    mProgressAnimator =
        ObjectAnimator.ofFloat(this, "progress", 0, 0).setDuration(DEFAULT_ANIMATION_DURATION);
    ...
    Drawable thumbDrawable =
        ResUtils.getDrawableAttrRes(
            context, typedArray, R.styleable.SwitchButton_swb_thumbDrawable);
    ColorStateList thumbColor =
        ResUtils.getColorStateListAttrRes(
            context, typedArray, R.styleable.SwitchButton_swb_thumbColor);
    margin = typedArray.getDimension(R.styleable.SwitchButton_swb_thumbMargin, margin);
    float marginLeft =
        typedArray.getDimension(R.styleable.SwitchButton_swb_thumbMarginLeft, margin);
    float marginRight =
        typedArray.getDimension(R.styleable.SwitchButton_swb_thumbMarginRight, margin);
    float marginTop = typedArray.getDimension(R.styleable.SwitchButton_swb_thumbMarginTop, margin);
    float marginBottom =
        typedArray.getDimension(R.styleable.SwitchButton_swb_thumbMarginBottom, margin);
    ...
    Drawable backDrawable =
        ResUtils.getDrawableAttrRes(context, typedArray, R.styleable.SwitchButton_swb_backDrawable);
    ColorStateList backColor =
        ResUtils.getColorStateListAttrRes(
            context, typedArray, R.styleable.SwitchButton_swb_backColor);
    float thumbRangeRatio =
        typedArray.getFloat(
            R.styleable.SwitchButton_swb_thumbRangeRatio, DEFAULT_THUMB_RANGE_RATIO);
    int animationDuration =
        typedArray.getInteger(
            R.styleable.SwitchButton_swb_animationDuration, DEFAULT_ANIMATION_DURATION);
    ...
    int textThumbInset =
        typedArray.getDimensionPixelSize(R.styleable.SwitchButton_swb_textThumbInset, 0);
    ...
    typedArray =
        context.obtainStyledAttributes(
            attrs, new int[] {android.R.attr.focusable, android.R.attr.clickable});
  ...
  private int measureWidth(int widthMeasureSpec) {
    ...
    if (widthMode == MeasureSpec.EXACTLY) {
      ...
      if (mThumbWidth != 0) {
        ...
        mBackWidth =
            ceil(moveRange + mThumbMargin.left + mThumbMargin.right + Math.max(textExtraSpace, 0));
        if (mBackWidth < 0) {
          mThumbWidth = 0;
        }
        if (moveRange
      ...
      if (mThumbWidth == 0) {
        contentSize = widthSize - getPaddingLeft() - getPaddingRight();
        moveRange =
            ceil(contentSize - Math.max(mThumbMargin.left, 0) - Math.max(mThumbMargin.right, 0));
        if (moveRange < 0) {
          mThumbWidth = 0;
          mBackWidth = 0;
          return measuredWidth;
          ...
          ceil(
              textWidth
                  ...
                  - (moveRange
                      ...
                      + mTextThumbInset));
      mBackWidth =
          ceil(moveRange + mThumbMargin.left + mThumbMargin.right + Math.max(0, textExtraSpace));
      if (mBackWidth < 0) {
        mThumbWidth = 0;
        mBackWidth = 0;
        return measuredWidth;
  ...
  private void setup() {
    ...
    int drawingHeight =
        ceil(mBackHeight - Math.min(0, mThumbMargin.top) - Math.min(0, mThumbMargin.bottom));
    ...
    mSafeRectF.set(
        mThumbRectF.left, 0, mBackRectF.right - mThumbMargin.right - mThumbRectF.width(), 0);
    ...
    if (mOffLayout != null) {
      ...
      mTextOffRectF.set(
          offLeft, offTop, offLeft + mOffLayout.getWidth(), offTop + mOffLayout.getHeight());
    }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (mIsBackUseDrawable) {
      if (mFadeBack && mCurrentBackDrawable != null && mNextBackDrawable != null) {
        ...
        // So make sure the order of on/off layers never change during slide from one endpoint to
        // another.
      ...
      if (mFadeBack) {
        ...
        mPaint.setARGB(
            colorAlpha, Color.red(belowColor), Color.green(belowColor), Color.blue(belowColor));
        ...
        colorAlpha = colorAlpha * alpha / 255;
        mPaint.setARGB(
            colorAlpha, Color.red(aboveColor), Color.green(aboveColor), Color.blue(aboveColor));
        canvas.drawRoundRect(mBackRectF, mBackRadius, mBackRadius, mPaint);
    ...
    if (switchText != null && textRectF != null) {
      ...
      switchText
          .getPaint()
          .setARGB(colorAlpha, Color.red(textColor), Color.green(textColor), Color.blue(textColor));
      canvas.save();
    ...
    if (mDrawDebugRect) {
      ...
      canvas.drawLine(
          mSafeRectF.left, mThumbRectF.top, mSafeRectF.right, mThumbRectF.top, mRectPaint);
