...
class DialogInit {
  ...
  public static void init(final MaterialDialog dialog) {
    ...
    if (builder.backgroundColor == 0) {
      builder.backgroundColor =
          ThemeUtils.resolveColor(
              ...
              ThemeUtils.resolveColor(
                  dialog.getContext(), androidx.appcompat.R.attr.colorBackgroundFloating));
    }
    if (builder.backgroundColor != 0) {
    ...
    if (builder.limitIconToDefaultSize
      ...
      maxIconSize =
          builder.context.getResources().getDimensionPixelSize(R.dimen.default_md_icon_max_size);
    }
    if (maxIconSize > -1) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
      ...
      if (textAllCaps) {
        textAllCaps =
            ThemeUtils.resolveBoolean(builder.context, androidx.appcompat.R.attr.textAllCaps, true);
      }
    } else {
      textAllCaps =
          ThemeUtils.resolveBoolean(builder.context, androidx.appcompat.R.attr.textAllCaps, true);
      ...
      if (builder.wrapCustomViewInScroll) {
        ...
        final int framePadding =
            ThemeUtils.resolveDimension(
                dialog.getContext(),
                R.attr.md_dialog_frame_margin,
                R.dimen.default_md_dialog_frame_margin);
    ...
    int windowVerticalPadding =
        ThemeUtils.resolveDimension(
            builder.context,
            R.attr.md_dialog_vertical_margin,
            ResUtils.getDimensionPixelSize(
                builder.context, R.dimen.default_md_dialog_vertical_margin_phone));
    int windowHorizontalPadding =
        ThemeUtils.resolveDimension(
            builder.context,
            R.attr.md_dialog_horizontal_margin,
            ResUtils.getDimensionPixelSize(
                builder.context, R.dimen.default_md_dialog_horizontal_margin_phone));
