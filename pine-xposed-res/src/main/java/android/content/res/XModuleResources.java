...
import v.android.content.res.AssetManagerKt;
...
public class XModuleResources extends Resources {
   ...
   *
   * <p>This is usually called with {@link StartupParam#modulePath} from {@link
   * IXposedHookZygoteInit#initZygote} and {@link InitPackageResourcesParam#res} from {@link
   * IXposedHookInitPackageResources#handleInitPackageResources} (or {@code null} for system-wide
   * replacements).
   *
  ...
  public static XModuleResources createInstance(String path, XResources origRes) {
    ...
    AssetManagerKt.addAssetPath(assets, path);

    XModuleResources res;
    if (origRes != null)
      res =
          new XModuleResources(
              assets,
              AssetManagerKt.getDisplayMetrics(assets),
              AssetManagerKt.getConfiguration(assets));
    else res = new XModuleResources(assets, null, null);
  ...
  /**
   * Creates an {@link XResForwarder} instance that forwards requests to {@code id} in this
   * resource.
   */
